        ...
        (*auth).send(move |client| {
            ...
            app_info(client, &app_id)
                ...
                .and_then(move |(access_container, app)| {
                    ...
                    access_container_entry(&c3, &access_container, &app.info.id, app.keys.clone())
                ...
                .and_then(move |(version, app, permissions, access_container)| {
                    ...
                    c4.mutate_mdata_entries(
                        access_container.name,
                        access_container.type_tag,
                        del.into(),
                    ).map(move |_| {
                            (app, app_entry_name, permissions, access_container)
                        })
                        .map_err(From::from)
                        .into_box()
                })
                ...
                .and_then(move |app| {
                    c8.list_auth_keys_and_version().map_err(From::from).map(
                        move |(_, version)| (version, app),
                    )
                })
