...
pub(super) struct ConnectionGroup {
    quic_p2p: QuicP2p, // cloned instance of a `QuicP2p` object (or `Arc<Mutex<QP2p>>` if needed)
    elders: Vec<NodeInfo>,
    hooks: HashMap<Token, Sender<Response>>, // to be replaced with Accumulator for multiple vaults.
}

impl ConnectionGroup {
    pub fn send(&mut self, msg: &[u8]) -> impl Future<Item = Response, Error = CoreError> {
        ...
        for node_info in self.elders.iter().cloned() {
            self.quic_p2p
                .send(Peer::Node { node_info }, Bytes::from(msg), token);
        }
