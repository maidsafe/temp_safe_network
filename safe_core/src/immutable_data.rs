...
mod tests {
    ...
    // Test gen_data_map, attempting to retrieve it
    // Generated address should match the one obtained when put to the network
    #[cfg(not(debug_assertions))]
    #[test]
    fn test_gen_data_map() {
        use crate::errors::CoreError;
        use safe_nd::Error as SndError;
        let value = unwrap!(utils::generate_random_vector(2 * 1024));
        ...
        random_client(move |client| {
            ...
            // gen address without putting to the network
                    ...
                    get_value(&client2, address_before, None)
                        .then(move |res| {
                            match res {
                                ...
                                Err(CoreError::DataError(SndError::NoSuchData)) => assert!(true),
                                Err(_) => panic!("Unexpected error when ImmutableData retrieved using address generated by gen_data_map"),
                            }
                            // let's put it to the network
                            create(&client3, &value_before2.clone(), true, None)
                        })
                        .then(move |res| {
                            let data = unwrap!(res);
                            let address_after = *data.address();
                            // the addresses generated without/with putting to the network should match
                            assert_eq!(address_after, address_before);
                            client4.put_idata(data).map(move |_| address_after)
                        })
                })
                .then(move |res| {
