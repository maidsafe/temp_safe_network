...
use sn_interface::{
    messaging::{data::chunk_operation_id, system::NodeQueryResponse},
...
impl ChunkStorage {
    ...
    pub(super) async fn get(&self, address: &ChunkAddress) -> NodeQueryResponse {
        ...
        match chunk_operation_id(address) {
            Ok(op_id) => NodeQueryResponse::GetChunk((
                self.get_chunk(address).await.map_err(|error| error.into()),
                op_id,
            )),
            Err(_) => NodeQueryResponse::FailedToCreateOperationId,
        }
    }
