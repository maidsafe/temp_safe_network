...
impl MyNode {
    pub(crate) async fn handle_join_request(
        ...
        let our_section_key = node.network_knowledge.section_key();
        ...
        if node.is_not_elder() && section_key_matches {
        ...
        if !our_prefix.matches(&peer.name()) {
            ...
            let retry_sap = node.matching_section(&peer.name())?;
        ...
        let (is_age_invalid, expected_age) = node.verify_joining_node_age(&peer)?;
        ...
        if !section_key_matches || is_age_invalid {
            let signed_sap = node.network_knowledge.signed_sap();
            let proof_chain = node.network_knowledge.section_chain();
    ...
    pub(crate) async fn handle_join_as_relocated_request(
            ...
            || join_request.section_key != node.network_knowledge.section_key()
        {
            ...
            let msg = NodeMsg::JoinAsRelocatedResponse(Box::new(JoinAsRelocatedResponse::Retry(
                node.network_knowledge.section_auth(),
            )));

            trace!("{} b", LogMarker::SendJoinAsRelocatedResponse);

            trace!("Sending {msg:?} to {peer}");
            return Some(node.send_system_msg(msg, Peers::Single(peer)));
        }
        ...
        let relocate_details = if let MembershipState::Relocated(ref details) = state {
            ...
            let known_keys = node.network_knowledge.known_keys();
        ...
        node.propose_membership_change(join_request.relocate_proof.value)
    }
}
