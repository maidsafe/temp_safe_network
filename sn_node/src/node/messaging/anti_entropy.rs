...
impl Node {
    ...
    pub(crate) fn send_ae_update_to_nodes(
        ...
        self.send_system(ae_msg, Peers::Multiple(recipients))
    }
    ...
    pub(crate) async fn handle_anti_entropy_retry_msg(
        ...
        match to_resend {
            ...
            Some((msg_to_resend, _)) => {
                ...
                result.push(self.send_system(msg_to_resend, Peers::Single(sender)));
        ...
        match to_resend {
            ...
            Some((msg_to_redirect, msg_id)) => match chosen_dst_elder {
                ...
                Some(elder) => {
                    ...
                    Ok(vec![self.send_system(msg_to_redirect, Peers::Single(elder))])
                }
            },
        }
    }
