...
use sn_interface::{
    messaging::{
        ...
        system::{AntiEntropyKind, NodeMsg, NodeMsgResponse},
...
impl MyNode {
    ...
    pub(crate) async fn handle_node_msg_with_ae_check(
        ...
        if context.is_elder {
            ...
            let ae_msg = match msg {
                NodeMsg::AntiEntropyUpdate { .. } | NodeMsg::JoinAsRelocatedRequest(_) => {
            ...
            if let Some(ae_msg) = ae_msg {
                let ae_cmd = if let Some(send_stream) = send_stream {
                    ...
                    Cmd::SendNodeMsgResponseOverUniStream {
                        msg: ae_msg,
                        correlation_id: msg_id,
                        recipients: Peers::Single(origin),
                        context,
                    }
                };
    ...
    pub(crate) fn send_ae_update_to_nodes(
        ...
        let ae_msg = NodeMsg::AntiEntropyUpdate {
            section_tree_update: MyNode::generate_ae_section_tree_update(context, Some(section_pk)),
            members,
        };

        MyNode::send_system_msg(ae_msg, recipients, context.clone())
    }

    /// Send `AntiEntropy` update response message to the specified nodes.
    pub(crate) fn send_ae_update_response(
        ...
        let ae_msg = NodeMsgResponse::AntiEntropy {
            section_tree_update: MyNode::generate_ae_section_tree_update(context, Some(section_pk)),
