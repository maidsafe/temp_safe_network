        ...
        rt.block_on(async {
            ...
            let node_ref = match new_node(config, join_timeout).await {
                ...
                Err(NodeError::JoinTimeout) => {
                    let join_retry_sec = JOIN_TIMEOUT_RETRY_TIME_SEC;
                    ...
                    error!("{message}");
                    sleep(Duration::from_secs(join_retry_sec));
                    return Ok(());
                }
                Err(error) => {
            ...
            // Defining address for our RPC service: we just usi the same as sn_node's IP
            let mut addr = node_ref.context.socket_addr();
            ...
            // otherwise both the node and the gRPC service will be running.
            ...
            match ctl_rx.recv().await {
                ...
                Some(NodeCtl::Restart(delay)) => {
                    println!("Node is restartig in {delay} seconds: {log_dir} ...");
        ...
        while let Ok(event) = events_rx.recv().await {
            match event {
                NodeEvent::ContextUpdated(new_context) => {
                    ...
                    // so the RPC services can read up to data info
