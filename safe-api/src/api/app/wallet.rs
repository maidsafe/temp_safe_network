...
mod tests {
    ...
    fn test_wallet_insert_and_balance() {
        ...
        async_std::task::block_on(async {
            ...
            let (_key1_xorurl, key_pair1) =
                unwrap!(safe.keys_create_preload_test_coins("12.23").await);
            let (_key2_xorurl, key_pair2) =
                unwrap!(safe.keys_create_preload_test_coins("1.53").await);
    ...
    fn test_wallet_insert_and_set_default() {
        ...
        async_std::task::block_on(async {
            let wallet_xorurl = unwrap!(safe.wallet_create().await);
            let (key1_xorurl, key_pair1) =
                unwrap!(safe.keys_create_preload_test_coins("65.82").await);
            let (key2_xorurl, key_pair2) =
                unwrap!(safe.keys_create_preload_test_coins("11.44").await);
    ...
    fn test_wallet_transfer_no_default() {
        ...
        async_std::task::block_on(async {
            ...
            let (_key_xorurl, key_pair) =
                unwrap!(safe.keys_create_preload_test_coins("43523").await);
            unwrap!(
                safe.wallet_insert(
                    &to_wallet_xorurl,
                    Some("my-first-balance"),
                ...
                .await
            {
                ...
                Ok(_) => {
                    let from_current_balance =
                        unwrap!(safe.wallet_balance(&from_wallet_xorurl).await);
    ...
    fn test_wallet_transfer_to_safekey() {
        ...
        async_std::task::block_on(async {
            ...
            let (key_xorurl, key_pair2) =
                unwrap!(safe.keys_create_preload_test_coins("10.0").await);
    ...
    fn test_wallet_transfer_from_specific_balance() {
        ...
        async_std::task::block_on(async {
            ...
            let (_key_xorurl2, key_pair2) =
                unwrap!(safe.keys_create_preload_test_coins("200.5").await);
            unwrap!(
                safe.wallet_insert(
                    &from_wallet_xorurl,
            ...
            let (_key_xorurl3, key_pair3) =
                unwrap!(safe.keys_create_preload_test_coins("10.5").await);
            unwrap!(
                safe.wallet_insert(
                    &to_wallet_xorurl,
    ...
    fn test_wallet_transfer_to_specific_balance() {
        ...
        async_std::task::block_on(async {
            let from_wallet_xorurl = unwrap!(safe.wallet_create().await);
            let (_key_xorurl1, key_pair1) =
                unwrap!(safe.keys_create_preload_test_coins("100.7").await);
            unwrap!(
                safe.wallet_insert(
                    &from_wallet_xorurl,
                    Some("from-first-balance"),
            ...
            let to_wallet_xorurl = unwrap!(safe.wallet_create().await);
            let (_key_xorurl2, key_pair2) =
                unwrap!(safe.keys_create_preload_test_coins("10.2").await);
            unwrap!(
                safe.wallet_insert(
                    &to_wallet_xorurl,
                    Some("to-first-balance"),
            ...
            let mut to_wallet_spendable_balance =
                unwrap!(XorUrlEncoder::from_url(&to_wallet_xorurl));
    ...
    fn test_wallet_transfer_specific_balances_with_nrs_urls() {
        ...
        let mut safe = Safe::default();
        unwrap!(safe.connect("", Some("fake-credentials")));
        let from_wallet_xorurl = async_std::task::block_on(async {
            let from_wallet_xorurl = unwrap!(safe.wallet_create().await);
            let (_key_xorurl1, key_pair1) =
        ...
        let to_wallet_xorurl = async_std::task::block_on(async {
            let to_wallet_xorurl = unwrap!(safe.wallet_create().await);
            let (_key_xorurl3, key_pair3) =
