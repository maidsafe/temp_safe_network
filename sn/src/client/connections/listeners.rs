...
use crate::types::{
    log_markers::LogMarker, utils::compare_and_write_prefix_map_to_disk, NamedPeer,
};
            ...
            loop {
                match Self::listen_for_incoming_msg(addr, &mut incoming_msgs).await {
                    Ok(Some(msg)) => {
                        if first {
                            first = false;
                            session.connections.add_incoming(&node_id, conn.clone()).await;
                        }
                    ...
                    if let Ok(op_id) = response.operation_id() {
                        if let Some(entry) = queries.get(&op_id) {
                            ...
                            for (_msg_id, sender) in all_senders {
                                //trace!("Internally relaying response for query w/{:?} via channel.", op_id);
                                let _res = sender.try_send(response.clone());
                                // if res.is_err() {
                                //     trace!("Error relaying query response internally on a channel for {:?} op_id {:?}: {:?}. (It has likely been removed)", msg_id, op_id, result)
                                // }
                            }
                        } else {
