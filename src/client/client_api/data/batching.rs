...
use super::pac_man::{get_data_chunks, get_file_chunks};
use super::{Db, ItemKey, Stash, ToDbKey};
use crate::client::utils::encryption;
use crate::client::{Error, Result};
use crate::dbs::{KvStore, Value};
use crate::types::{Chunk, ChunkAddress};
use crate::UsedSpace;
use crate::{messaging::data::RegisterWrite, url::Scope};
use bytes::Bytes;
...
async fn process_values(
    ...
    let batches: Vec<_> = values
        ...
        .map(|(key, (data, scope))| {
            ...
            let (_root_address, chunks) = get_data_chunks(data.clone(), owner.as_ref())?;
...
async fn process_files(
    ...
    let batches: Vec<_> = files
        ...
        .map(|(key, (path, scope))| {
            let owner = encryption(*scope, get_random_pk());
            let (_root_address, chunks) = get_file_chunks(path, owner.as_ref())?;
            Ok::<_, Error>((key.clone(), chunks))
        })
        .flatten()  // drops errors
        .collect();
