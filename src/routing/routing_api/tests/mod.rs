...
async fn aggregate_proposals() -> Result<()> {
    ...
    let core = Core::new(
        create_comm().await?,
        nodes[0].clone(),
        Section::from(section.clone().await),
        Some(section_key_share),
        mpsc::channel(TEST_EVENT_CHANNEL_SIZE).0,
        used_space,
        root_storage_dir,
...
async fn handle_untrusted_message(source: UntrustedMessageSource) -> Result<()> {
    ...
    let mut section_dto = section.clone().await;
...
async fn handle_sync() -> Result<()> {
    ...
    let wire_msg = WireMsg::single_src(
        ...
        NodeMsg::Sync {
            section: new_section.clone().await,
            network: NetworkDto::new(),
        },
        section_pk,
    )?;
...
async fn handle_bounced_untrusted_sync() -> Result<()> {
    ...
    let core = Core::new(
        create_comm().await?,
        node.clone(),
        Section::from(section_full.clone().await),
        Some(section_key_share),
        event_tx,
        used_space,
        root_storage_dir,
    ...
    let original_node_msg = NodeMsg::Sync {
        section: section_full.clone().await,
        network: NetworkDto::new(),
    };
...
async fn handle_elders_update() -> Result<()> {
    ...
    let core = Core::new(
        create_comm().await?,
        node,
        Section::from(section0.clone().await),
        Some(section_key_share),
        event_tx,
        used_space,
        root_storage_dir,
