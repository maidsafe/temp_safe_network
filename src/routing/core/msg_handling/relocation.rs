...
use crate::routing::{
    ...
    error::Result,
    peer::PeerUtils,
    relocation::{self, RelocateAction, RelocateDetailsUtils, RelocationStatus},
...
impl Core {
    pub(crate) async fn relocate_peers(
        ...
        let relocations =
            relocation::actions(&self.section, &self.network, churn_name, churn_signature).await;
    ...
    pub(crate) async fn handle_relocate(
        ...
        if let Some(status) = self.relocation_state.get() {
            match status.as_ref() {
                RelocationStatus::InProgress(_) => {
                    trace!("Ignore Relocate - relocation already in progress");
                    return Ok(None);
                }
                RelocationStatus::Delayed(_) => (),
            }
        } else {
        ...
        let joining_as_relocated = JoiningAsRelocated::new(
            self.node.clone(),
            genesis_key,
            relocate_details,
            node_msg,
        ...
        let cmd = joining_as_relocated.start(bootstrap_addrs).await?;

        self.relocation_state
            .set(RelocationStatus::InProgress(Box::new(joining_as_relocated)));
    ...
    pub(crate) async fn handle_relocate_promise(
        ...
        if promise.name == self.node.name() {
            ...
            if self.relocation_state.get().is_some() {
        ...
        if promise.name == self.node.name() {
            ...
            if let Some(status) = self.relocation_state.get() {
                ...
                self.relocation_state
                    .set(RelocationStatus::Delayed(msg.clone()));
                self.send_event(Event::RelocationStarted {
                    previous_name: self.node.name(),
                })
                .await;
            }
            // match self.relocation_state.get() {
            //     None => {
            //         trace!("Received RelocatePromise to section at {}", promise.dst);
            //         self.relocation_state
            //             .set(RelocationStatus::Delayed(msg.clone()));
            ...
            //         .await;
            //     }
            //     Some(RelocationStatus::InProgress(_)) => {
            //         trace!("ignore RelocatePromise - relocation already in progress");
            //     }
            //     Some(RelocationStatus::Delayed(_)) => {
            //         trace!("ignore RelocatePromise - already have one");
            //     }
            // }
        ...
        if let Some(info) = self.section.members().get(&promise.name).await {
            let details =
