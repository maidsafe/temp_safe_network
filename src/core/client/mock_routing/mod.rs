...
mod test {
    ...
    use routing::{AppendWrapper, AppendedData, Authority, Data, DataIdentifier, Event, Filter,
                  FullId, ImmutableData, MessageId, PubAppendableData,
                  Response, StructuredData, XOR_NAME_LEN, XorName};
    use routing::client_errors::{GetError, MutationError};
    use rust_sodium::crypto::sign;
    ...
    fn do_put(routing: &mut MockRouting,
        ...
        match unwrap!(routing_rx.recv()) {
            ...
            Event::Response {
                response: Response::PutFailure { id, data_id, external_error_indicator }, ..
            } => {
                assert_eq!(id, message_id);
                let reason = unwrap!(deserialise(&external_error_indicator));
    ...
    fn do_post(routing: &mut MockRouting,
        ...
        match unwrap!(routing_rx.recv()) {
            ...
            Event::Response {
                response: Response::PostFailure { id, data_id, external_error_indicator }, ..
            } => {
                assert_eq!(id, message_id);
                let reason = unwrap!(deserialise(&external_error_indicator));
    ...
    fn do_delete(routing: &mut MockRouting,
        ...
        match unwrap!(routing_rx.recv()) {
            ...
            Event::Response {
                response: Response::DeleteFailure { id, data_id, external_error_indicator }, ..
            } => {
                assert_eq!(id, message_id);
                let reason = unwrap!(deserialise(&external_error_indicator));
    ...
    fn do_append(routing: &mut MockRouting,
        ...
        match unwrap!(routing_rx.recv()) {
            ...
            Event::Response {
                response: Response::AppendFailure { id, data_id, external_error_indicator }, ..
            } => {
                assert_eq!(id, message_id);
                let reason = unwrap!(deserialise(&external_error_indicator));
    ...
    fn do_get_account_info(routing: &mut MockRouting,
        ...
        match unwrap!(routing_rx.recv()) {
            Event::Response {
                response: Response::GetAccountInfoSuccess { id, data_stored, space_available }, ..
            } => {
                assert_eq!(id, message_id);
