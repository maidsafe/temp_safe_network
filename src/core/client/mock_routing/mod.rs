...
// cases. Once activated
// the GET's should only be performed once success from PUT's/POST's/DELETE's
// have been obtained.
//
// These will allow to code properly for behavioural anomalies like GETs
// reaching the address faster
// than PUTs. So a proper delay will help code better logic against scenarios
// where it is required
// to do a GET after a PUT/DELETE to confirm that action. So for example if a
...
impl MockRouting {
    ...
    // clients can direct
    // data to wherever they want. It is only the requirement of maidsafe-routing
    // that GET's should
    ...
    pub fn send_put_request(&self,
        ...
        // assume that
        // MaidManagers always pass the PUT. Errors if any can come only from
        // NaeManagers
        let nae_auth = Authority::NaeManager(data_name);
