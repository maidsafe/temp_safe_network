...
enum SplitStage {
    Pending {
        next_actor_state: RewardsAndWallets,
...
impl SectionFunds {
    ...
    //     let new_wallet = network.section_public_key().await.ok_or(Error::NoSectionPublicKey)?;
    ...
    //             let next_actor = Self::get_actor(replicas, network.clone()).await?;
    ...
    //                     queued_ops.push(NetworkDuty::from(NodeMessagingDuty::Send(OutgoingMsg {
    //                         msg: Message::NodeCmd {
    //                             cmd: Transfers(PropagateTransfer(proof.clone())),
    //                             id: MessageId::new(),
    //                             target_section_pk: None,
    //                         },
    //                         section_source: false,
