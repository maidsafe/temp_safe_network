...
use crate::api::xorurl::{SafeDataType, SafeContentType, XorUrlEncoder as NativeXorUrlEncoder};
...
use super::ffi_structs::XorUrlEncoder;
...
pub unsafe extern "C" fn new_xor_url_encoder_from_xor_url(
    ...
    catch_unwind_cb(user_data, o_cb,  || -> ResultReturn<()> {
        ...
        let ffi_encoder = xor_url_encoder.into_repr_c()?;
        o_cb(user_data.0, FFI_RESULT_OK, &ffi_encoder);
        Ok(())
    })
}
