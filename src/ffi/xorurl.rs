...
pub unsafe extern "C" fn encode_xor_url(
    ...
    catch_unwind_cb(user_data, o_cb,  || -> ResultReturn<()> {
        ...
        let url_path = from_c_str(path)?;
        let encoding_base = from_c_str(base_encoding)?;
        let encoded_xor_url = XorUrlEncoder::encode(xor_name, type_tag, data_type_enum, content_type_enum, Some(&url_path), Some(vec![]), Some(content_version), &encoding_base)?; //todo: update sub_names parameter
        let encoded_string = CString::new(encoded_xor_url).map_err(|_| Error::Unexpected("Couldn't convert to string".to_string()))?;
...
pub unsafe extern "C" fn new_xor_url_encoder(
    ...
    catch_unwind_cb(user_data, o_cb,  || -> ResultReturn<()> {
        ...
        let content_type_enum = SafeContentType::from_u64(content_type);
        let url_path = from_c_str(path)?;
        let encoder = XorUrlEncoder::new(xor_name, type_tag, data_type_enum, content_type_enum, Some(&url_path), Some(vec![]), Some(content_version)); //todo: update sub_names parameter
