...
// fn delete_file(app: &App, file_path: &str, is_shared: bool) -> Result<(),
// FfiError> {
//     let (mut directory, file_name) =
//         try!(helper::dir_and_file(app, file_path, is_shared));

// let _ = try!(file_helper::delete(app.client(), file_name, &mut
// directory));
...
// try!(helper::get_directory_and_file(app, src_path,
// is_src_path_shared));
// let mut dst_dir = try!(helper::get_directory(app, dst_path,
// is_dst_path_shared));
...
//     if retain_src {
// file = try!(File::new(file.get_metadata().clone(),
