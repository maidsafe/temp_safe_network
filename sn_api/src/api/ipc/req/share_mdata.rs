...
use sn_data_types::{MapPermissionSet, XorName};
...
impl ShareMapReq {
    ...
    pub fn into_repr_c(self) -> Result<ffi::ShareMapRequest, IpcError> {
        ...
        let (mdata, map_len) = vec_into_raw_parts(map_repr_c);
...
impl ReprC for ShareMapReq {
    ...
    unsafe fn clone_from_repr_c(repr_c: Self::C) -> Result<Self, Self::Error> {
        Ok(Self {
            ...
            mdata: {
                let mdata = slice::from_raw_parts((*repr_c).mdata, (*repr_c).map_len);
                mdata
                    .iter()
