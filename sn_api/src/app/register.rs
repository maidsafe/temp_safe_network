...
use safe_network::{
    ...
    types::{
        ...
        DataAddress, Error as SafeNdError, PublicKey, RegisterAddress, Scope,
    },
};
...
impl Safe {
    ...
    pub async fn register_create(
        ...
        let my_pk = self.client.public_key();
...
fn private_policy(owner: PublicKey) -> Policy {
    let mut permissions = BTreeMap::new();
    let _ = permissions.insert(owner, PrivatePermissions::new(true, true));
    Policy::Private(PrivatePolicy { owner, permissions })
}

fn public_policy(owner: PublicKey) -> Policy {
    let mut permissions = BTreeMap::new();
    let _ = permissions.insert(User::Key(owner), PublicPermissions::new(true));
