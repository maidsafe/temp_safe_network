...
mod tests {
    ...
    async fn test_nrs_associate_with_topname() -> Result<()> {
        ...
        let nrs_map = retry_loop_for_pattern!(safe.nrs_get_subnames_map(&site_name, None,flow_name), Ok(nrs_map) if !nrs_map.map.is_empty())?;
    ...
    async fn test_nrs_associate_with_subname() -> Result<()> {
        ...
        let nrs_map = retry_loop_for_pattern!(safe.nrs_get_subnames_map(&site_name, None,flow_name), Ok(nrs_map) if nrs_map.map.len() == 1)?;
    ...
    async fn test_nrs_conflicting_names() -> Result<()> {
        ...
        let conflict_error = retry_loop_for_pattern!( safe.nrs_get(&site_name, None,flow_name), res if res.is_err() );
        assert!(
