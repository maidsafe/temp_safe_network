...
pub struct SectionKeysProvider {
    ...
    cache: Rc<RefCell<LRUCache<SectionKeyShare, KEY_CACHE_SIZE>>>,
}
...
impl SectionKeysProvider {
    ...
    pub fn new(current: Option<SectionKeyShare>) -> Self {
        let section_keys_provider = Self {
            cache: Rc::new(RefCell::new(LRUCache::default())),
        };
    ...
    pub fn insert(&self, share: SectionKeyShare) {
        let public_key = share.public_key_set.public_key();
        if let Some(evicted) = self.cache.borrow_mut().insert(share) {
